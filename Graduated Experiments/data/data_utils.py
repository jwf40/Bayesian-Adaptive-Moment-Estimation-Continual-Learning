...
def get_CIsplitmnist(batch_size=128, **kwargs):
    n_tasks= 5 if not kwargs['n_tasks'] else kwargs['n_tasks']#badam_mean_eta: 0.1, badam_std: 0.01
    kwargs.update({'n_classes':10, 'hidden':200, \
...
def get_CIsplitfmnist(batch_size=128, **kwargs):
    n_tasks= 5 if not kwargs['n_tasks'] else kwargs['n_tasks']#badam_mean_eta: 0.1, badam_std: 0.01
    kwargs.update({'n_classes':10, 'hidden':200, \
              'badam_mean_eta': 0.1, 'badam_std': 0.01,\
              'bgd_mean_eta': 10, 'bgd_std': 0.017,\
    ...
    train_dsets = _split(FashionMNIST(root="~/.torch/data/fashionmnist", train=True, download=True), n_classes=10, n_splits=n_tasks)
    test_dsets = _split(FashionMNIST(root="~/.torch/data/fashionmnist", train=False, download=True), n_classes=10, n_splits=n_tasks)
