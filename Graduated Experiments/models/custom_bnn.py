...
from .custom_bayes_linear import BayesLinear
...
class MLP(nn.Module):
    def __init__(self, n_samples=50, n_tasks=3):    
        ...
        self.model = nn.Sequential(BayesLinear(prior_mu=0, prior_sigma=0.1, in_features=28*28, out_features=100),\
                                    nn.ReLU(),
                                    BayesLinear(prior_mu=0, prior_sigma=0.1, in_features=100, out_features=100),
                                    nn.ReLU())

        #Head to solve tasks
        self.prediction_head = nn.Sequential(BayesLinear(prior_mu=0, prior_sigma=0.1, in_features=100, out_features=10))
        
        #Head to matchmake
        self.matchmaking_head = nn.Sequential(BayesLinear(prior_mu=0, prior_sigma=0.1,in_features=100+(28*28), out_features=100),
                                BayesLinear(prior_mu=0, prior_sigma=0.1,in_features=100, out_features=n_tasks))
