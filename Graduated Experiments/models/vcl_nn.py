"""
Adapted from:
https://github.com/Jack742/variational-continual-learning
"""
...
class MLP(nn.Module):
    ...
    def get_prediction_means(self, x,num_samples=TRAIN_NUM_SAMPLES):
        means_ = []
        for i in range(num_samples):
            means_.append(nn.Softmax(dim=1)(self.forward(x, 0)))
        means_ = torch.stack(means_)
        means_ = means_.mean(dim=0)
        return means_

    def predictive_entropy(self,x,num_samples=TRAIN_NUM_SAMPLES):
        eps = 1e-10
        means_ = self.get_prediction_means(x, num_samples)
        entropy = means_*torch.log(torch.add(means_,eps))
        return -entropy.sum(dim=1)

    def predictive_entropy_from_means(self,means_):
        eps = 1e-10
        entropy = means_*torch.log(torch.add(means_,eps))
        return -entropy.sum(dim=1)


    def single_forward_pass(self, x, head=0, num_samples=TEST_NUM_SAMPLES):
        return nn.Softmax(dim=1)(self.forward(x,head))
