...
class BayesLinear(Module):
    ...
    def __init__(self, prior_mu, prior_sigma, in_features, out_features, bias=True):
        ...
        self.weights=Parameter(torch.Tensor(out_features, in_features), requires_grad=True)
        self.biases=Parameter(torch.Tensor(out_features), requires_grad=True)
        # self.weights.retain_grad()
        # self.biases.retain_grad()
        
        # self.weights.register_hook(lambda grad: grad)
        # self.biases.register_hook(lambda grad: grad)
        ...
        self.register_buffer('weight_eps', torch.randn_like(self.weight_log_sigma))
        ...
        if self.bias:
            ...
            self.register_buffer('bias_eps', torch.randn_like(self.bias_log_sigma))
    ...
    def forward(self, input):
        ...
        self.weight_eps = torch.randn_like(self.weight_log_sigma)
        self.weights = Parameter(self.weight_mu + torch.exp(self.weight_log_sigma) * self.weight_eps)      

        self.bias_eps = torch.randn_like(self.bias_log_sigma)
        self.biases = Parameter(self.bias_mu + torch.exp(self.bias_log_sigma) * self.bias_eps)
