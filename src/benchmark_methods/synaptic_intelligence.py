...
class SI(BaseCLMethod):
    ...
    def _init_experience(self):
        ...
        for param_name, param in params:
            if param_name not in self.ewc_data[0]:
                ...
                self.syn_data["old_theta"][param_name] = torch.zeros_like(
                    param.flatten()
                )
                self.syn_data["new_theta"][param_name] = torch.zeros_like(
                    param.flatten()
                )
                self.syn_data["grad"][param_name] = torch.zeros_like(param.flatten())
                self.syn_data["trajectory"][param_name] = torch.zeros_like(
                    param.flatten()
                )
                self.syn_data["cum_trajectory"][param_name] = torch.zeros_like(
                    param.flatten()
                )
                ...
                self.syn_data["trajectory"][param_name].expand(param.flatten().shape)
                self.syn_data["cum_trajectory"][param_name].expand(
                    param.flatten().shape
                )
    ...
    def _get_allowed_params(self):
        ...
        def _get_layers_and_params(module=self.model, prefix=""):
            ...
            for ln, layer in module.named_modules():
                ...
                result += _get_layers_and_params(
                    module=layer, prefix=layer_complete_name
                )
    ...
    def _calc_reg(self):
        ...
        for n, p in params:
            ...
            syn_loss = torch.dot(ewc_data1, (weights - ewc_data0) ** 2) * (lambda_ / 2)
            loss += syn_loss
        return loss
    ...
    def _update_importances(self, c=0.0015, data=None):
        ...
        for n in self.syn_data["cum_trajectory"]:
            self.syn_data["cum_trajectory"][n].data += (
                ...
                * self.syn_data["trajectory"][n].data
                / (
                    np.square(
                        self.syn_data["new_theta"][n].data - self.ewc_data[0][n].data
                    )
                    + self.eps
                )
            )
        ...
        for param_name in self.ewc_data[1]:
            ...
            self.ewc_data[0][param_name].data = self.syn_data["new_theta"][
                param_name
            ].data.clone()
